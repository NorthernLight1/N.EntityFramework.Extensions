        ...
        public static int BulkInsert<T>(this DbContext context, IEnumerable<T> entities, BulkInsertOptions<T> options)
        {
            ...
            using (var dbTransactionContext = new DbTransactionContext(context))
            {
                try
                {
                    ...
                    IEnumerable<string> autoGeneratedColumnNames = options.AutoMapOutputIdentity ? tableMapping.GetAutoGeneratedColumns(options.KeepIdentity) : new string[] { };
            ...
            using (var dbTransactionContext = new DbTransactionContext(context))
            {
                try
                {
                    ...
                    foreach (var primaryKeyColumnName in primaryKeyColumnNames)
                    {
                        ...
                        var storedGeneratedColumn = tableMapping.Columns.First(o => o.Column.Name == primaryKeyColumnName);
